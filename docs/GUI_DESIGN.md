...
Status: written before implementation and kept in step with it. Sections 12 and 13 record what was
measured and what building it found. All seven phases are built.
...
| 3 | done: React + TypeScript + Vite + Tailwind on CSS tokens, zustand store, canvas viewport with eight overlay layers, nine-stage stepper, stage drawer, outcome card, always-visible safety rail, dark and light themes, keyboard map | verified in headless Chromium; e-stop 3.2 ms median while moving, 10 fps front camera |
| 4 | done: Inspector with a 10 Hz scrubber, joint angles against targets, fingertip charts, latency waterfall, replay into Live Run; vitest over the projection maths, the formatters and the run indexing | 15 frontend tests |
| 5 | done: Results built from `results/*.json` with Wilson bars and per-file provenance; Batch Evaluate through `langgrasp.eval.harness` with a live grid, cancel and overwrite protection | API and integration tests, including a real three-trial job |
| 6 | done: Safety & System from `langgrasp/gui/fmea.yaml`, checked against docs/FMEA.md, the code and the test suite by `tests/test_gui_fmea.py`; microphone flow; axe-core and responsive passes | axe-core: 0 violations across five views and two themes. Lighthouse: accessibility 100, best practices 100 |
| 7 | done: nine Playwright checks against a real worker, `make gui`, `make gui-build`, `make gui-test`, `make gui-e2e`, `make gui-audit`, `make gate` extended, README and DEMO_GUIDE rewritten with screenshots | 9 e2e checks pass; `results/*.json` unchanged |
...
## 13. What building it found

The GUI was also a test of the stack underneath. Each of these was found by running the thing, not by reading
it, and each is fixed with a test that fails without the fix:

| Found | Where it is pinned down |
|---|---|
| Enforcing the safety monitor's 1.5 rad/s velocity limit changes the executor's trajectory: 120/120 placements observing against 73/120 enforcing, with 35% of ticks clipped. The comment beside `max_joint_vel` claimed the executor peaked well below the limit. | `scripts/audit_safety_clips.py`, `results/safety_clip_audit.json` |
| Grounding DINO on CUDA is not bit-reproducible: two identical runs differ by about 0.002 px of box and 5e-4 of score, with identical decisions. | `tests/test_gui_hooks_identical.py` |
| Loading Grounding DINO takes 6 s, which the camera watchdog read as a sensor fault and latched an e-stop before the first command could run. | `langgrasp/gui/worker.py`, phase 2 |
| Batch trials tripped the camera watchdog mid-motion, because this stack captures one frame per command and then executes open loop. The first trial stopped half way through and every trial after it failed without moving. | `tests/test_gui_watchdog.py` |
| A batch job never refreshed the command watchdog, so a job started after an idle spell ran every trial with the arm held. | `tests/test_gui_watchdog.py` |
| Five `results/*.json` files contain bare `NaN`, which Python writes and no browser will parse: every ACT row rendered as "not run" for a measurement that exists. | `tests/test_gui_api.py` |
| The FMEA table cited a test that had been renamed. | `tests/test_gui_fmea.py` |
| The Live Run controls were rendered twice, once per breakpoint, so every form id existed twice. | `gui/e2e/app.spec.ts` |
| A recorded run kept only every fifth frame at speeds above real time, so the Inspector's scrubber had holes at exactly the ticks someone would want to look at. | phase 7 |
| Three zustand selectors built a new object per call, which looped React and left the page blank. | phase 3 |

Two things this GUI deliberately does not claim:

* a live run is not a protocol measurement. It runs on a GPU shared with other processes, with instrumentation
  attached, and the UI labels its latencies as such while showing the clean bench beside them;
* the watchdogs measure this worker's own sampling gaps, not an independent publisher, because there is no
  camera node here. The Safety view says so in those words.

## 14. A second Phase 0 draft

A shorter design document for the same brief appeared in the working tree during phase 6 and was committed by
accident in `049afc3`; this file was restored from `641525d`. Nothing was lost: that draft is readable with
`git show 049afc3:docs/GUI_DESIGN.md`. Where the two disagree, this one is what was built. The clearest
difference is the responsive target: that draft puts layouts below 1024 px out of scope, while this build
works down to 420 px with no element extending past the viewport.
